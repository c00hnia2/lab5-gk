...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {
		...
		glut.glutSolidCube(17); //Minimalne zmniejszy�em wielko�� podstawy by piramida nie by�a tak du�a
		gl2.glPopMatrix();
		...
		float x,z,x1,z1;
		int i; //Do p�tli rysuj�cej boki
		int n = 4; //Ilo�� bok�w 
		int color=5; //Pozwala zmieni� kolor
		int size =12;
		float y = -1; //Wysoko�� na jakiej startuje rysowanie
		x1 = (float) Math.sin(0)*size;
		z1= (float) Math.cos(0)*size;
		
		//Dodawanie materia��w i ich warto�ci
		...
		gl2.glMaterialf( GL2.GL_FRONT_AND_BACK, GL2.GL_SHININESS, materials[color][12] ); //W tej konfiguracji piramida b�dzie przechodzi�a w kolor jasnor�owy
		
		for (i = 1; i<= n; i += 1) { //P�tla do rysowania bok�w
			...
			gl2.glVertex3f(0, 8, 0); //Warto�c y odpowiedzialna za wysoko��
			}
		gl2.glEnd();
		gl2.glFlush();
		//Koniec rysowania piramidy
		
	//Tworzenie zr�d�a �wiat�a
		
    	//Dodanie pseudo s�o�ca
	
		  float sun[] = { 1F, 1, 0, 1 };
		  gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, gray, 0); 
          gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_EMISSION, sun, 0);  //Sprawiaj� �e kulka jest ��ta
          gl2.glTranslated(8, 6, 0); //Ustawia s�o�ce w przestrzeni
          gl2.glEnable(GL2.GL_LIGHT1); //Zaczyna emitowa� ��te �wiat�o

          glut.glutSolidSphere(3, 16, 8); //Wielko�� kulki
          gl2.glPopMatrix();
          
          //Dodanie pomara�czowej kuli
          
          float orange[] = {1, 0.5F, 0, 1 };
          gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_EMISSION, orange, 0); 
